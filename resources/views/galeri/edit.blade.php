...
@section('title', 'Edit Galeri Sekolah')

@section('content')
  <div class="page-header d-flex justify-content-between align-items-center mb-4">
    <div>
      <h2 class="mb-1 fw-bold text-dark" style="letter-spacing: -0.5px;">Edit Galeri Sekolah</h2>
      <p class="text-muted text-sm mb-0">Form untuk mengubah dokumentasi foto atau video kegiatan.</p>
    </div>
      ...
      <form action="{{ route('galeri.update', $galeri->id_galeri) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" value="{{ old('judul', $galeri->judul) }}" placeholder="Contoh: Upacara Bendera" required>
            @error('judul')
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="date" class="form-control @error('tanggal') is-invalid @enderror" name="tanggal" value="{{ old('tanggal', $galeri->tanggal) }}" required>
            @error('tanggal')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-6 mb-3">
            ...
            <div class="form-text text-muted">Biarkan kosong jika tidak ingin mengubah file. Format: JPEG, PNG, JPG, MP4, MKV (Maks. 10MB).</div>
            @error('file')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-12 mb-3">
            ...
            <textarea class="form-control @error('keterangan') is-invalid @enderror" name="keterangan" rows="4" placeholder="Tuliskan keterangan lengkap kegiatan..." required>{{ old('keterangan', $galeri->keterangan) }}</textarea>
            @error('keterangan')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
